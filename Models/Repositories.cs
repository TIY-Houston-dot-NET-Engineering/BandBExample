...
public class Repo<T> : IRepository<T> where T : class, HasId {
    ...
    public Repo(DB db, string tableName, Func<DbSet<T>,IEnumerable<T>> includer){
        this.db = db;
        dbtable = GetTable(tableName); // db[tableName]
        table = includer(dbtable); // includer(db[tableName]) --> db[tableName].Include(...)
        // table.Where()... -> db[tableName].Include(...).Where(...)
    }
    ...
    public IEnumerable<T> Read(Func<DbSet<T>, IEnumerable<T>> fn){ //bandbRepo.Read(dbset => dbset.Include().Where()...)
